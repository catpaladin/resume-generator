...
<div class="space-y-4">
  ...
  {:else}
    ...
    {:else}
        <div class="space-y-3">
            {#each resumeStore.resumeData.experience as exp (exp.id)}
                <div class="border-2 rounded-xl px-8 py-5 bg-card opacity-50">
                    <div class="flex flex-col items-start gap-1">
                        <span class="font-bold text-slate-900 dark:text-slate-100">{exp.position || '(No Position)'}</span>
                        <span class="text-xs text-slate-500">{exp.company || 'Company Name'}</span>
                    </div>
                </div>
            {/each}
        </div>
