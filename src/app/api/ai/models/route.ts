...
async function fetchGeminiModels(apiKey: string) {
  ...
  const generationModels = data.models
    ?.filter((model: { supportedGenerationMethods?: string[]; name: string }) => 
      model.supportedGenerationMethods?.includes('generateContent') &&
      model.name.includes('gemini')
    )
    .map((model: { name: string; description?: string; inputTokenLimit?: number }) => ({
      id: model.name.replace('models/', ''),
      name: formatModelName(model.name.replace('models/', '')),
      description: model.description || getModelDescription(model.name.replace('models/', ''), 'gemini'),
      context_length: model.inputTokenLimit,
      isRecommended: isRecommendedModel(model.name.replace('models/', ''), 'gemini'),
      isDeprecated: isDeprecatedModel(model.name.replace('models/', ''), 'gemini'),
    })) || [];
