  ...
  async enhanceResume(
    ...
    try {
      ...
      const messages: OpenAIMessage[] = [
        {
          role: "system",
          content: `You are a professional resume enhancement expert. Your task is to improve resume content while maintaining accuracy and authenticity. 
...
Response format:
{
  ...
  private buildEnhancementPrompt(request: AIEnhancementRequest): string {
    let prompt = `Please enhance this resume data while maintaining factual accuracy:\n\n`;

    prompt += `Original Resume Text:\n${request.originalText}\n\n`;

    prompt += `Current Parsed Data:\n${JSON.stringify(request.parsedData, null, 2)}\n\n`;
    ...
    if (request.focusAreas?.length) {
      prompt += `Focus Areas: ${request.focusAreas.join(", ")}\n\n`;
    }

    prompt += `Enhancement Level: ${request.enhancementLevel}\n\n`;
